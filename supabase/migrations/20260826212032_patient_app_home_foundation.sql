...
begin
  ...
  select
    ...
    pe.cycle_number,
    pe.current_phase_id,
    coalesce(ppt.title, null),
    pe.current_session_id,
    coalesce(pst.title, null),
    pe.last_activity_at,
    case
...
comment on function public.get_patient_app_home(text) is 'M12 patient app home read model. Returns only the authenticated user own active programme rights and open enrollment state. No health questionnaire answers are exposed or persisted.';