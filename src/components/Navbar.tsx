      ...
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-border bg-background/98 backdrop-blur-xl shadow-[0_1px_10px_rgba(15,45,34,0.025)]">
        <div className="container mx-auto flex items-center justify-between gap-5 h-[104px] px-4">
          ...
          <div className="hidden lg:flex shrink-0 items-center gap-3 ml-6">
            <Link to="/cart" aria-label={cartLabel} className="relative grid h-9 w-9 place-items-center rounded-lg border border-[#00572f] bg-[#00572f] text-white hover:border-accent hover:bg-accent transition-colors">
            ...
            <div className="flex items-center gap-1 border border-border rounded-lg bg-background px-1.5 py-1">
              ...
              {langs.map((l) => <button key={l.code} onClick={() => setLang(l.code)} className={`text-[11px] font-bold px-2 py-1 rounded-md text-foreground transition-colors ${lang === l.code ? "bg-secondary" : "hover:bg-accent hover:text-accent-foreground"}`}>{l.label}</button>)}
            </div>
            <div className="flex flex-col gap-1">
              <Button size="sm" className="h-7 bg-[#00572f] px-4 font-bold text-white hover:bg-accent hover:text-accent-foreground" asChild><Link to="/finder">{tr(t.nav.startNow)}</Link></Button>
          ...
          <button className="lg:hidden grid h-10 w-10 place-items-center rounded-lg border border-border bg-background hover:border-accent hover:bg-accent hover:text-accent-foreground transition-colors" onClick={() => setOpen(!open)} aria-label={open ? "Close menu" : "Open menu"}>
        ...
        {open && (
          <div className="lg:hidden bg-background border-t border-border px-4 py-4 shadow-soft">
            <div className="space-y-0.5">
              {links.map((l) => <Link key={l.to} to={l.to} onClick={() => setOpen(false)} className="block rounded-lg px-3 py-2.5 text-sm font-semibold text-muted-foreground hover:bg-accent hover:text-accent-foreground">{l.label}</Link>)}
              {user && <Link to="/patient" onClick={() => setOpen(false)} className="block rounded-lg px-3 py-2.5 text-sm font-semibold text-muted-foreground hover:bg-accent hover:text-accent-foreground">{patientLabel}</Link>}
            </div>
            <div className="mt-4 pt-4 border-t border-border">
              <Link to="/cart" onClick={() => setOpen(false)} aria-label={cartLabel} className="flex items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-semibold text-muted-foreground hover:bg-accent hover:text-accent-foreground"><ShoppingCart className="w-4 h-4" aria-hidden="true" />{tr({ fr: "Panier", en: "Cart", de: "Warenkorb" })}{itemCount > 0 && <span className="grid h-5 min-w-5 place-items-center rounded-full bg-primary px-1.5 text-[10px] font-bold text-primary-foreground">{itemCount}</span>}</Link>
              <div className="flex items-center gap-2 px-3 mt-3">{langs.map((l) => <button key={l.code} onClick={() => setLang(l.code)} className={`text-xs font-bold px-3 py-1.5 rounded-lg border transition-colors ${lang === l.code ? "border-primary/20 bg-secondary text-primary" : "border-border bg-background text-muted-foreground hover:border-accent hover:bg-accent hover:text-accent-foreground"}`}>{l.label}</button>)}</div>
