...
/**
 * Converts transient M06 answers from question UUIDs to stable keys expected by
 * the trusted server-side checkout function. Nothing is persisted here.
 */
export function buildSafetyAnswerPayload(
  ...
  for (const question of applicableQuestions) {
    ...
    if (typeof answer !== "boolean") {
      throw new TrustedCheckoutError(
        "missing_safety_answer",
        `Missing safety answer for ${question.stableKey}.`,
      );
    }
...
/**
 * M08 trusted checkout preparation.
 *
 * The browser sends programme IDs and transient safety answers only. It never
 * sends authoritative prices, totals, currency, payment state or entitlement.
 * Supabase recomputes catalogue values and safety server-side and creates only
 * a PENDING order snapshot. Payment is deliberately deferred to M09.
 */
export async function createTrustedCheckoutOrder(input: TrustedCheckoutInput): Promise<TrustedCheckoutOrder> {
  ...
  const programmeIds = [...new Set(input.programmeIds)].filter(
  ...
  if (error) {
    throw new TrustedCheckoutError(classifyCheckoutError(error.message), error.message);
  }

  const row = Array.isArray(data) ? data[0] : data;
  if (!row) throw new TrustedCheckoutError("unknown", "Trusted checkout returned no order.");

  if (String(row.order_status) !== "pending") {
    throw new TrustedCheckoutError("unknown", "Trusted checkout returned an invalid order state.");
  }

  const totalAmount = Number(row.total_amount);
  if (!Number.isInteger(totalAmount) || totalAmount < 0) {
    throw new TrustedCheckoutError("unknown", "Trusted checkout returned an invalid total.");
  }
