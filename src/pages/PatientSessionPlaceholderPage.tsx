...
const PatientSessionPlaceholderPage = () => {
  ...
  useEffect(() => {
    ...
    fetchPatientSessionExperience(enrollmentId, lang)
      ...
      .catch((err) => {
        if (!cancelled) setError(err instanceof Error ? err.message : "session.load_failed");
      })
      .finally(() => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background">
        <Navbar />
        <main className="pt-28 pb-20"><div className="container mx-auto px-4 max-w-7xl"><div className="rounded-3xl border border-border bg-card p-8" aria-busy="true">{tr({ fr: "Chargement de votre séance…", en: "Loading your session…", de: "Ihre Sitzung wird geladen…" })}</div></div></main>
        <Footer />
      </div>
    );
  }
        ...
        <main className="pt-28 pb-20">
          <div className="container mx-auto px-4 max-w-2xl">
            <div className="rounded-3xl border border-border bg-card p-8 text-center">
              <ShieldAlert className="w-10 h-10 mx-auto text-primary mb-4" />
              <h1 className="text-3xl font-heading font-bold mb-3">{tr({ fr: "Séance non disponible", en: "Session unavailable", de: "Sitzung nicht verfügbar" })}</h1>
              <p className="font-body text-muted-foreground mb-6">{tr({ fr: "Cette séance n’est pas encore publiée ou votre compte ne dispose pas de l’accès requis.", en: "This session is not yet published or your account does not have the required access.", de: "Diese Sitzung ist noch nicht veröffentlicht oder Ihr Konto verfügt nicht über den erforderlichen Zugriff." })}</p>
              <Button asChild><Link to="/patient">{tr({ fr: "Retour à mon espace", en: "Back to my space", de: "Zurück zu meinem Bereich" })}</Link></Button>
            </div>
          </div>
        </main>
        <Footer />
      ...
      <main className="pt-24 pb-16">
        <div className="container mx-auto px-4 max-w-[1480px]">
          <div className="flex flex-wrap items-center justify-between gap-3 py-5">
            ...
            <div className="rounded-full border border-border bg-card px-4 py-2 text-sm font-body text-muted-foreground">
              {tr({ fr: "Exercice", en: "Exercise", de: "Übung" })} {activeIndex + 1} / {session.exercises.length}
            </div>
          </div>
          ...
          <div className="grid xl:grid-cols-[300px_minmax(0,1fr)] gap-5 items-start">
            <aside className="hidden xl:block rounded-3xl border border-border bg-card p-4 sticky top-24 max-h-[calc(100vh-7rem)] overflow-y-auto">
              <div className="mb-4 px-2">
                <p className="text-xs uppercase tracking-wide font-body font-semibold text-primary">{tr({ fr: "Séance", en: "Session", de: "Sitzung" })}</p>
                <h2 className="font-heading font-bold text-xl mt-1">{session.sessionName}</h2>
                {session.estimatedDurationMinutes && <p className="text-sm text-muted-foreground font-body mt-1"><Clock3 className="inline w-4 h-4 mr-1" />{session.estimatedDurationMinutes} min</p>}
              </div>
              <div className="space-y-2">
                {session.exercises.map((item, index) => (
                  <button key={item.prescriptionId} type="button" onClick={() => go(index)} className={`w-full text-left rounded-2xl border p-3 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${index === activeIndex ? "border-primary bg-primary/5" : "border-border hover:bg-muted/50"}`} aria-current={index === activeIndex ? "step" : undefined}>
                    <div className="flex gap-3 items-center">
                      <div className="relative w-20 aspect-video rounded-xl overflow-hidden bg-muted shrink-0">
                        {item.placeholderImage ? <img src={item.placeholderImage} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gradient-to-br from-primary/10 to-muted" />}
                        <span className="absolute inset-0 grid place-items-center"><span className="grid place-items-center w-9 h-9 rounded-full bg-primary text-primary-foreground shadow"><Play className="w-4 h-4 fill-current" /></span></span>
                      </div>
                      <div className="min-w-0"><p className="text-sm font-body font-semibold line-clamp-2">{index + 1}. {item.name}</p><p className="text-xs text-muted-foreground mt-1">{item.sets ? `${item.sets}×` : ""}{item.repetitions ?? ""}{item.holdSeconds ? ` · ${item.holdSeconds}s` : ""}</p></div>
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div className="space-y-5 min-w-0">
                ...
                <div className="grid lg:grid-cols-[minmax(0,1.45fr)_minmax(330px,.9fr)]">
                  <div className="p-4 md:p-6">
                    <div className="relative aspect-video rounded-2xl overflow-hidden bg-muted border border-border">
                      ...
                      <button type="button" className="absolute inset-0 m-auto w-20 h-20 md:w-24 md:h-24 rounded-full bg-primary text-primary-foreground grid place-items-center shadow-xl focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/50" aria-label={tr({ fr: "Vidéo de démonstration à venir", en: "Demonstration video coming later", de: "Demonstrationsvideo folgt später" })} onClick={() => undefined}>
                        <Play className="w-9 h-9 md:w-11 md:h-11 fill-current translate-x-0.5" />
                      </button>
              ...
              <section className="rounded-3xl border border-border bg-card p-4 md:p-5">
                ...
                <div className="flex gap-3 overflow-x-auto pb-2 snap-x">
                  {session.exercises.map((item, index) => <button key={item.prescriptionId} type="button" onClick={() => go(index)} className={`min-w-[220px] max-w-[220px] snap-start text-left rounded-2xl border overflow-hidden bg-background transition ${index === activeIndex ? "border-primary ring-1 ring-primary/20" : "border-border"}`}><div className="relative aspect-video bg-muted">{item.placeholderImage && <img src={item.placeholderImage} alt="" className="w-full h-full object-cover" />}<span className="absolute inset-0 grid place-items-center"><span className="grid place-items-center w-11 h-11 rounded-full bg-primary text-primary-foreground shadow"><Play className="w-5 h-5 fill-current" /></span></span></div><div className="p-3"><p className="font-body font-semibold text-sm line-clamp-2">{index + 1}. {item.name}</p></div></button>)}
                </div>
              </section>
...
const InfoCard = ({ title, children, warning = false }: { title: string; children: React.ReactNode; warning?: boolean }) => <article className={`rounded-2xl border p-5 ${warning ? "border-amber-500/25 bg-amber-500/5" : "border-border bg-card"}`}><h3 className="font-heading font-bold text-lg flex items-center gap-2 mb-3">{warning && <AlertTriangle className="w-5 h-5 text-amber-600" />}{title}</h3><div className="space-y-3 font-body text-sm text-muted-foreground leading-relaxed">{children}</div></article>;
