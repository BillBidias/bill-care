...
const PatientHomePage = () => {
  ...
  const actionLabel = (programme: PatientHomeProgramme) => {
    if (programme.todayAction === "resume_session") return tr({ fr: "Reprendre ma séance", en: "Resume my session", de: "Sitzung fortsetzen" });
    if (programme.todayAction === "resume_programme") return tr({ fr: "Reprendre le programme", en: "Resume programme", de: "Programm fortsetzen" });
    if (programme.todayAction === "start_programme") return tr({ fr: "Démarrer le programme", en: "Start programme", de: "Programm starten" });
    return tr({ fr: "Contenu en préparation", en: "Content being prepared", de: "Inhalt in Vorbereitung" });
  };

  const statusLabel = (programme: PatientHomeProgramme) => {
    if (programme.enrollmentStatus === "active") return tr({ fr: "En cours", en: "In progress", de: "In Bearbeitung" });
    if (programme.enrollmentStatus === "paused") return tr({ fr: "En pause", en: "Paused", de: "Pausiert" });
    return tr({ fr: "Non commencé", en: "Not started", de: "Nicht begonnen" });
  };
        ...
        <div className="container mx-auto px-4 max-w-7xl">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
              <div>
                <p className="text-sm font-body font-semibold text-primary mb-2">{tr({ fr: "Espace patient", en: "Patient space", de: "Patientenbereich" })}</p>
                <h1 className="text-3xl md:text-4xl font-heading font-bold">{tr({ fr: "Que dois-je faire aujourd’hui ?", en: "What should I do today?", de: "Was soll ich heute tun?" })}</h1>
                <p className="mt-2 text-muted-foreground font-body max-w-2xl">{tr({ fr: "Retrouvez vos programmes achetés et reprenez votre parcours là où vous l’avez laissé.", en: "Find your purchased programmes and continue where you left off.", de: "Finden Sie Ihre gekauften Programme und setzen Sie dort fort, wo Sie aufgehört haben." })}</p>
              </div>
              <Button variant="outline" asChild><Link to="/account">{tr({ fr: "Mon profil", en: "My profile", de: "Mein Profil" })}</Link></Button>
            </div>

            {loading ? (
              <div className="rounded-2xl border border-border bg-card p-8" aria-busy="true"><p className="font-body text-muted-foreground">{tr({ fr: "Chargement de vos programmes…", en: "Loading your programmes…", de: "Programme werden geladen…" })}</p></div>
            ) : error ? (
              <div className="rounded-2xl border border-destructive/30 bg-card p-6"><p role="alert" className="text-sm text-destructive font-body mb-4">{error}</p><Button variant="outline" onClick={() => void load()}>{tr({ fr: "Réessayer", en: "Try again", de: "Erneut versuchen" })}</Button></div>
            ) : programmes.length === 0 ? (
              <div className="rounded-3xl border border-border bg-card p-8 md:p-10 text-center"><LockKeyhole className="w-10 h-10 mx-auto mb-4 text-muted-foreground" /><h2 className="text-2xl font-heading font-bold mb-2">{tr({ fr: "Aucun programme dans votre compte", en: "No programme in your account", de: "Noch kein Programm in Ihrem Konto" })}</h2><p className="font-body text-muted-foreground mb-6">{tr({ fr: "Utilisez le Program Finder ou consultez les programmes disponibles pour trouver le parcours adapté.", en: "Use the Program Finder or browse available programmes to find a suitable path.", de: "Nutzen Sie den Program Finder oder sehen Sie sich verfügbare Programme an." })}</p><Button asChild><Link to="/programs">{tr({ fr: "Voir les programmes", en: "Browse programmes", de: "Programme ansehen" })}</Link></Button></div>
            ) : (
              <>
                {today && (
                  <section className="rounded-3xl border border-primary/20 bg-card p-6 md:p-8 shadow-sm mb-10">
                    <div className="grid md:grid-cols-[1fr_auto] gap-6 items-center"><div><p className="text-sm font-body font-semibold text-primary mb-2">{tr({ fr: "Votre prochaine action", en: "Your next action", de: "Ihre nächste Aktion" })}</p><h2 className="text-2xl font-heading font-bold mb-2">{today.programmeTitle}</h2>{today.currentPhaseTitle && <p className="text-sm font-body text-muted-foreground">{today.currentPhaseTitle}</p>}{today.currentSessionTitle && <p className="text-base font-body font-semibold mt-1">{today.currentSessionTitle}</p>}</div><Button size="lg" disabled={today.todayAction === "content_not_ready" || workingId === today.programmeId} onClick={() => void runAction(today)} className="min-w-48"><Play className="w-4 h-4 mr-2" />{actionLabel(today)}</Button></div>
                  </section>
                )}
